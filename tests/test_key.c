...
static int gpio_reads[GPIO_NUM_MAX];
...
static int init_configures_one_pullup_input(void)
{
    key_t keys = {.initialized = false};
    CHECK(key_init(&keys, KEY_GPIO) == ESP_OK);
    CHECK(keys.initialized);
    CHECK(config_calls == 1 && read_calls == 1);
    CHECK(gpio_reads[KEY_GPIO] == 1 && gpio_reads[26] == 0);
    CHECK(last_config.pin_bit_mask == (1ULL << KEY_GPIO));
...
static int init_rejects_invalid_arguments(void)
{
    ...
    CHECK(key_init(NULL, KEY_GPIO) == ESP_ERR_INVALID_ARG);
    for (size_t i = 0; i < sizeof(invalid) / sizeof(invalid[0]); ++i) {
        CHECK(key_init(&keys, invalid[i]) == ESP_ERR_INVALID_ARG);
    }
...
static int init_rejects_duplicate_without_reconfiguring(void)
{
    key_t keys = {.initialized = false};
    CHECK(key_init(&keys, KEY_GPIO) == ESP_OK);
    CHECK(key_init(&keys, 32) == ESP_ERR_INVALID_STATE);
    CHECK(config_calls == 1 && read_calls == 1);
    levels[KEY_GPIO] = 0;
    EXPECT_POLL(&keys, 100, KEY_EVENT_NONE);
    EXPECT_POLL(&keys, 30100, KEY_EVENT_PRESSED);
    return 0;
}
...
static int init_gpio_failure_can_retry(void)
{
    ...
    CHECK(key_init(&keys, KEY_GPIO) == ESP_FAIL);
    CHECK(!keys.initialized && read_calls == 0 && clock_calls == 0);
    config_result = ESP_OK;
    CHECK(key_init(&keys, KEY_GPIO) == ESP_OK);
    CHECK(config_calls == 2 && read_calls == 1);
    levels[KEY_GPIO] = 0;
    EXPECT_POLL(&keys, 100, KEY_EVENT_NONE);
    EXPECT_POLL(&keys, 30100, KEY_EVENT_PRESSED);
    return 0;
...
{
    key_t keys = {.initialized = false};
    key_event_t event = KEY_EVENT_PRESSED;
    CHECK(key_poll(NULL, &event) == ESP_ERR_INVALID_ARG);
    CHECK(event == KEY_EVENT_PRESSED);
...
static int press_requires_full_30ms(void)
{
    key_t keys = {.initialized = false};
    CHECK(key_init(&keys, KEY_GPIO) == ESP_OK);
    levels[KEY_GPIO] = 0;
    ...
    EXPECT_POLL(&keys, 40000, KEY_EVENT_PRESSED);
    return 0;
}
...
static int press_bounce_restarts_debounce(void)
{
    ...
    EXPECT_POLL(&keys, 61000, KEY_EVENT_PRESSED);
    return 0;
}
...
static int held_button_never_repeats(void)
{
    ...
    levels[KEY_GPIO] = 0;
    EXPECT_POLL(&keys, 0, KEY_EVENT_NONE);
    EXPECT_POLL(&keys, 30000, KEY_EVENT_PRESSED);
...
static int release_bounce_does_not_rearm(void)
{
    ...
    EXPECT_POLL(&keys, 180000, KEY_EVENT_PRESSED);
    return 0;
}

static int release_then_press_again(void)
{
    key_t keys = {.initialized = false};
    CHECK(key_init(&keys, KEY_GPIO) == ESP_OK);
    levels[KEY_GPIO] = 0;
    ...
    EXPECT_POLL(&keys, 110000, KEY_EVENT_PRESSED);
    return 0;
}

static int boot_held_key_requires_stable_release(void)
{
    key_t keys = {.initialized = false};
    levels[KEY_GPIO] = 0;
    CHECK(key_init(&keys, KEY_GPIO) == ESP_OK);
    ...
    EXPECT_POLL(&keys, 250000, KEY_EVENT_PRESSED);
    return 0;
}

static int two_objects_have_independent_debounce_state(void)
{
    key_t first = {.initialized = false}, second = {.initialized = false};
    CHECK(key_init(&first, KEY_GPIO) == ESP_OK);
    CHECK(key_init(&second, 33) == ESP_OK);
    levels[KEY_GPIO] = 0;
    EXPECT_POLL(&first, 1000, KEY_EVENT_NONE);
    levels[33] = 0;
    EXPECT_POLL(&second, 20000, KEY_EVENT_NONE);
    EXPECT_POLL(&first, 31000, KEY_EVENT_PRESSED);
    EXPECT_POLL(&second, 31000, KEY_EVENT_NONE);
    EXPECT_POLL(&second, 50000, KEY_EVENT_PRESSED);
...
static int debounce_uses_64bit_microsecond_clock(void)
{
    ...
    EXPECT_POLL(&keys, start + 30000, KEY_EVENT_PRESSED);
    EXPECT_POLL(&keys, start + INT64_C(4294967296), KEY_EVENT_NONE);
    return 0;
}

static int unrelated_gpio26_does_not_generate_events(void)
{
    key_t key = {0};
    CHECK(key_init(&key, KEY_GPIO) == ESP_OK);
    levels[26] = 0;
    EXPECT_POLL(&key, 1000, KEY_EVENT_NONE);
    EXPECT_POLL(&key, 31000, KEY_EVENT_NONE);
    levels[26] = 1;
    EXPECT_POLL(&key, 100000, KEY_EVENT_NONE);
    levels[KEY_GPIO] = 0;
    EXPECT_POLL(&key, 110000, KEY_EVENT_NONE);
    EXPECT_POLL(&key, 140000, KEY_EVENT_PRESSED);
    CHECK(gpio_reads[KEY_GPIO] == read_calls && gpio_reads[26] == 0);
    return 0;
}
...
static const struct test_case cases[] = {
    TEST(init_configures_one_pullup_input),
    ...
    TEST(release_then_press_again),
    TEST(boot_held_key_requires_stable_release),
    TEST(two_objects_have_independent_debounce_state),
    TEST(debounce_uses_64bit_microsecond_clock),
    TEST(unrelated_gpio26_does_not_generate_events),
};
